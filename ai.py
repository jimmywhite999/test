...
FILENAME = '/home/ubuntu/chatgpt.txt'
        ...
        if float(result['update_id']) > float(last_update):
            ...
            if not result['message']['from']['is_bot']:
                ...
                if chat_id in ALLOWED_GROUP_ID:
                    ...
                    # Checking that user mentioned ChatGPT's username in message
                    if CHATBOT_HANDLE in result['message']['text']:
                        prompt = result['message']['text'].replace(CHATBOT_HANDLE, "")
                        boolean_active = True
                        
                    # Verifying that the user is responding to ChatGPT
